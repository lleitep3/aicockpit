...
func NewDoctorCommand(log *logging.Manager, cfg *config.Config, t *i18n.Translator) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runDoctor(cmd, log, cfg, t, jsonOutput)
		},
	}
...
func runDoctor(cmd *cobra.Command, log *logging.Manager, cfg *config.Config, t *i18n.Translator, jsonOutput bool) error {
	...
	// Signal telemetry failure via annotation so decorateCommands records status=error
	if !allOk {
		if cmd.Annotations == nil {
			cmd.Annotations = make(map[string]string)
		}
		cmd.Annotations["telemetry_status"] = "error"
	}
