...
// ── InstallPackage — SavePackage error branch ─────────────────────────────────

// TestInstallPackage_SavePackageFails exercises the SavePackage error path
// (lines 58-61) by making the install dir read-only after copyPackageFiles.
// Since SavePackage calls os.WriteFile on the manifest, we create a file
// where cockpit-package.yml should go, making WriteFile fail.
func TestInstallPackage_SavePackageFails(t *testing.T) {
	if os.Getuid() == 0 {
		t.Skip("skipping permission-based test when running as root")
	}
	...
	// Build a valid package source with no files to copy (empty skills dir),
	// so copyPackageFiles succeeds, but the skill file exists on disk for Validate.
	pkgDir := filepath.Join(tmpDir, "save-fail-src")
	if err := os.MkdirAll(filepath.Join(pkgDir, "skills"), 0o755); err != nil {
		t.Fatalf("setup: %v", err)
	}
	if err := os.WriteFile(filepath.Join(pkgDir, "skills", "skill.md"), []byte("# Skill"), 0o644); err != nil {
	...
	manifest := `name: "save-fail-pkg"
version: "1.0.0"
description: "SaveFail install test"
author: "Test"
license: "MIT"
type: "utility"
requirements:
...
`
	...
	// Pre-create the install path dir and put a directory where the manifest
	// should be written — WriteFile on a directory path fails.
	installedPath := filepath.Join(pm.GetPackagesDir(), "save-fail-pkg")
	manifestBlocker := filepath.Join(installedPath, "cockpit-package.yml")
	if err := os.MkdirAll(manifestBlocker, 0o755); err != nil {
		t.Fatalf("setup blocker: %v", err)
	}

	err := pm.InstallPackage(pkgDir, nil)
	if err == nil {
		t.Error("expected error when SavePackage cannot write manifest")
	}
	// The cleanup path should have removed the installed dir.
	if _, statErr := os.Stat(installedPath); statErr == nil {
		// Cleanup might not work if the dir is a blocker — that's ok for this test.
		t.Logf("note: installed dir still exists (cleanup may have failed due to blocker dir)")
	}
}
