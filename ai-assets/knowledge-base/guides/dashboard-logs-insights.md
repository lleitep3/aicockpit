...
Este documento descreve o funcionamento do sistema de Logs e Insights do Dashboard do Cockpit.
...
Todo comando executado no CLI é capturado pelo decorador central (`decorateCommands` em [root.go](file:///home/lleite/projects/aicockpit/cmd/root.go)). As métricas são armazenadas de forma persistente em `~/.cockpit/metrics.json`.
...
- `error` / `error_type`: Mensagem e tipo de erro Go, se aplicável.
